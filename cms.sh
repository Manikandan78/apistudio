...
[Service]                                                                                                                   WorkingDirectory=$HOME/nix_project/NANOX-API-STUDIO-master/CmsPage                                                          ExecStart="$HOME/nix_project/NANOX-API-STUDIO-master/CmsPage/venv/bin/uvicorn main:app --host 127.0.0.1 --port 8008 --worke>Restart=always
User=$USER
...
[Unit]
Description=CMS Page API Service
After=network.targetecho "✅ Systemd service file created at: $SERVICE_FILE"
