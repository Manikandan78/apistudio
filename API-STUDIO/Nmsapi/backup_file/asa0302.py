...
from config import DB_USER_API1, DB_NAME_API1, DB_PASSWORD_API1

def custom_api():
    db_user = DB_USER_API1
    db_name = DB_NAME_API1
    db_password = DB_PASSWORD_API1
    
    current_date = datetime.now().strftime("%d-%m-%Y-%H-%M %p")
    backup_dir = "/Backup/ETLSTUDIO"
    backup_log_dir = "/Backup/ETLSTUDIO/LOG"
    ...
    backup_file = f"{backup_dir}/ETLSTUDIO-{timestamp}.sql"
    log_file = f"{backup_log_dir}/ETLSTUDIO-{timestamp}.log"
