...
def check_service_status_data_migr():
    try:
        ...
        # Parse the result to determine the status of Apache and MySQL
        output = result.stdout.lower()
        apache_status = 'running' if 'apache is running' in output else 'not running'
        mysql_status = 'running' if 'mysql is running' in output else 'not running'
        return apache_status, mysql_status
...
# Example usage to check Apache and MySQL service status
apache_status, mysql_status = check_service_status_data_migr()
update_service_status_to_db('APACHE SERVER', 'apache', 'NGINX', apache_status)
update_service_status_to_db('MYSQL SERVER', 'mysql', 'NGINX', mysql_status)
