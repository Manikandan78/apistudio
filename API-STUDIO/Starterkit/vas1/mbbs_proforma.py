...
def mbbs_application_navigation(request):
    ...
    return redirect('mbbspplication_update', check_app['psk_id'])
...
def delete_mbbs_phc(request, psk_id, rec_parent_id):
    ...
    if response.status_code == 200:
        status = "UN-ALLOCATED"
        institution.update_phc_status(phc_id, status)
        messages.success(request, message="Deleted Successfully")
...
def mbbs_phc_form(request, rec_parent_id):
    ...
    if request.method == 'POST':
        ...
        academic_no_of_student = request.POST.get('academic_no_of_student') or 0
        ...
        if check_phc['phc_status'] == "UN-ALLOCATED":
            ...
            if response.status_code == 200:
                # Displaying success message and redirecting
                status = "ALLOCATED"
                institution.update_phc_status(name_of_phc, status)
                messages.success(request, message='Data Added successfully!')
                return redirect('mbbs_phc_form', rec_parent_id=rec_parent_id)
        else:
            # messages.info(request, message=f"PHC Already Allocated '{check_phc['phc_name']}'")
            messages.info(request, message=f"PHC '{check_phc['phc_name']}' is already allocated. Please use an "
                                           f"unallocated PHC")
...
def mbbs_check_list(request, rec_parent_id):
    ...
    if request.method == 'POST':
        for key, data in check_list_data.items():
            ...
            checklist_status = bool(request.POST.get(key))  # True if checked, False if unchecked
