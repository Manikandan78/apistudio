...
def anm_application(request):
    ...
    return render(request, "proforma/anm/anm_application.html")
...
def anm_student(request, rec_parent_id):
    ...
    if request.method == 'POST':
        ...
        # API endpoint URL
        dc1url = f"{API_STUDIO_URL}postapi/create/{Proforma_ANM_Student}"
        headers = {'Content-Type': 'application/json'}
...
def anm_phc_form(request, rec_parent_id):
    ...
    return render(request, "proforma/anm/anm_phc_form.html")


def anm_check_list(request, rec_parent_id):
    return render(request, 'proforma/anm/anm_check_list.html')