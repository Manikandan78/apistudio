...
@router.post('/crudapp/upload_multiple_media_files/media/{api_name}',tags=['Media Model Creation'])
async def upload_multiple_media_files(
    ...
    try:
        for idx, media in enumerate(media_files):
            # Read file contents and get MIME type
            contents = await media.read()
            media_type = mime.from_buffer(contents)

            # Update filename with a custom naming convention (e.g., "file_<index>_<original_name>")
            updated_filename = media.filename

            # Create and populate the database object
            obj = model(
                file_blob=contents,
            ...
            # Add parent_psk_id if provided and valid
            ...
            # Add successful upload info to response list
...
@router.put('/crudapp/update_multiple_media_files/media/{api_name}', tags=['Media Model Creation'])
async def update_multiple_media_files(
    api_name: str,
    media_files: List[UploadFile],
    psk_ids: str = Form(...),  # Accept psk_ids as a comma-separated string
    db: Session = Depends(get_db)
):
    try:
        # Convert comma-separated psk_ids into a list of integers
        psk_ids_list = [int(psk_id.strip()) for psk_id in psk_ids.split(",")]

        if len(media_files) != len(psk_ids_list):
            raise HTTPException(
                status_code=400,
                detail="The number of media files must match the number of psk_ids."
            )

        model = get_model(api_name)  # Assuming you have a function to fetch the model dynamically
        mime = magic.Magic(mime=True)
        updated_files = []

        for idx, (media, psk_id) in enumerate(zip(media_files, psk_ids_list)):
            try:
                # Fetch the existing object using psk_id
                ...
                if not obj:
                    updated_files.append({
                        ...
                        "message": "No object found with the provided psk_id"
                    })
                    continue

                # Read file contents and determine MIME type
                contents = await media.read()
                media_type = mime.from_buffer(contents)
                ...
                obj.file_name = updated_filename

                # Add successful update info to response list
                updated_files.append({
                    "psk_id": obj.psk_id,
                    "original_filename": media.filename,
                    "updated_filename": updated_filename,
    ...
    return {
        ...
        "message": "File update processing completed"
    }
...
@router.delete('/crudapp/delete_all_media_records/v2/{api_name}/{parent_psk_id}',
               tags=['Media Model Creation Version2'])
    ...
    try:
        ...
        if hasattr(gen_models, model_name):
            ...
            if records.count() > 0:
                ...
                db.commit()
                return {
                    "detail": f"All records for {api_name} with parent_psk_id {parent_psk_id} have been deleted successfully."}
            else:
